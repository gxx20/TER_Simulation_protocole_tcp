...
public class TcpGui {
	...
	private void show() {
		...
		JLabel subtitle = new JLabel(
				"Handshaking, transferts, retransmissions et export CSV dans une interface interactive.");
		...
		runBtn.addActionListener((ActionEvent e) -> {
			...
			Thread worker = new Thread(() -> {
				try {
					...
					double corruption = parseDoubleOrDefault(corruptionField.getText(),
		...
		exportBtn.addActionListener((ActionEvent e) -> {
			...
			try {
				...
				if (!Files.exists(out)) {
					Files.writeString(out, TransferSummary.csvHeader() + System.lineSeparator(),
	...
	private static void applyPreset(JComboBox<String> presetBox, JTextField packetsField, JTextField windowField,
			JTextField corruptionField, JTextField lossField) {
	...
	private static void addMetric(JPanel panel, int x, int y, String title, JLabel value) {
		...
		card.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(new Color(223, 229, 236)),
